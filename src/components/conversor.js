    ...
    render() { 
        return ( //Todos componentes deve retornar apenas uma div, e todos meus elementos deve estar dentro dessa div
            <div className="conversor"> {/*Componente conversor*/}
                ...
                <input id="in" type="text" onChange={(event) => {this.setState({moedaA_valor:event.target.value})}}></input>{/*Valor que vamos converter / onChange receberá uma function e vai atualizar o estado sempre que o campo mudar / Explicação: event.target = onChange irá disparar um evento, esse evento irá pegar quem é o target (nesse caso, será o ELEMENTO INPUT) e o valor que estiver dentro dele é o que nós queremos*/}
                <input id="btn" type="button" value="Converter" onClick={this.converter}></input>{/*onClick irá chamar a função converter*/}
